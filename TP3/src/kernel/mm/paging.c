...
PRIVATE int allocf(void)
{
	...
	for (i=0;i < NR_FRAMES;i++){
		thispte = getpte(curr_proc,frames[i].addr);
		if (thispte->dirty || thispte->accessed){
			frames[i].score+=100*(thispte->dirty+thispte->accessed);
		}
		else{
			frames[i].score=frames[i].score/1.1;
		}
	}
	...
	for (i = 0; i < NR_FRAMES; i++)
	{
		...
		if (frames[i].owner == curr_proc->pid)
		{
			...
			// /* Oldest page found. */
			if(frames[i].score < lowestscore || lowestpage < 0 || (frames[i].score == lowestscore && frames[i].age < frames[lowestpage].age)){
                lowestscore = frames[i].score;
                lowestpage = i;
      }
	...
	/* Swap page out. */
	if (swap_out(curr_proc, frames[i = lowestpage].addr)){
		frames[i].score=1000;
		return (-1);
	}
